...
class ItemSubCommand(SubCommandBase):
    ...
    def retrieve(self):
        self.fetch_and_set_soup(f'https://na.op.gg/champion/{self.get_champ_name_no_space()}/statistics/{self.champ_lane}/item?')
    ...
    def print(self):
        ...
        print("{:>7} {:>25} {:>12} {:>12}".format("index", "item name", "win rate", "pick rate"))
        for i, item_info in enumerate(self.item_info_list_sorted):
            if i < 10:
                ...
                print("{:>7} {:>25} {:>12} {:>12}".format(i+1, name, win_rate, pick_rate))
            else:
                break
